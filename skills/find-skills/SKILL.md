---
name: find-skills
description: 用于查询、安装 Agent Skill 并进行统一安全扫描。支持查询小艺Skill市场及外部开源Skill生态。当已安装skill无法满足用户需求，请调用此skill寻找skill。或者当用户明确要求搜索特定功能的skill、安装新skill时（如：“查询Skill”、“查找Skill”、“搜索Skill”、“安装Skill”、“添加Skill”、“find skill”、“search skill”、“install skill”等），请调用此skill。
---

# Skill查询、安装与安全扫描

此Skill用于发现并为 Agent 安装新能力，并在下载完成后统一强制执行安全扫描。它整合了本地小艺Skill市场和外部开源Skill生态。

## Skill查询与安装原则

1. 优先查询和安装小艺Skill。只有当小艺Skill完全无法满足用户要求时，才使用 npx 去查询和安装外部Skill。
2. 遵循“先查询、后评估过滤、再展示确认、安装、最后安全扫描”的完整闭环流程。
3. 外部Skill推荐时，需参考安装量和来源可靠性，并严格执行质量验证准则。
4. **注意：** 外部Skill查询严格使用英文关键词，小艺Skill查询严格使用中文关键词。
...
外部Skill：
- 查询：`npx skills find "[query]"`
- 安装：`npx skills add <package>`
- 检查更新：`npx skills check` - 检查外部Skill更新
- 更新Skill：`npx skills update` - 更新所有已安装的外部Skill
...
```bash
python scripts/search.py --query "<中文query>"
```
...
若未达到 2 次内部搜索上限：更换中文关键词重新执行第二步的内部搜索；
若已达到 2 次内部搜索上限：立即停止内部搜索，执行外部查询（严格使用英文关键词）。

```bash
npx skills find "[English_query]" 
```
...
- 安装外部Skill（`-g` 用于全局安装，`-y` 用于跳过确认）：
```bash
npx skills add <package_name> -g -y
```
